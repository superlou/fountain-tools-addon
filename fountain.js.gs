// Pulled from fountain-js 0.1.10
// http://www.opensource.org/licenses/mit-license.php
// Copyright (c) 2012 Matt Daly

// Modified dialogue regex to allow "#" in character name
// Modified to preserve tabs in action blocks
...
var lexer_with_whitespace = function (script) {
  return script.replace(regex.boneyard, '\n$1\n')
  .replace(regex.standardizer, '\n')
  .replace(regex.cleaner, '')
  .replace(/\t/g, '    ')
...
var tokenize = function (script) {
  ...
  var src_with_whitespace = lexer_with_whitespace(script).split(regex.splitter);
  var i      = src.length, line, match, parts, text, meta, x, xlen, dual;
  var tokens = [];
  
  while (i--) {
    line = src[i];
    line_with_whitespace = src_with_whitespace[i];
    ...
    tokens.push({ type: 'action', text: line_with_whitespace });
  }
